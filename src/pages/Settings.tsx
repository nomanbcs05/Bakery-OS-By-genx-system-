    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Tabs defaultValue="access" className="space-y-6">
        <TabsList className="bg-slate-100 p-1 rounded-xl h-auto flex flex-wrap gap-1">
          <TabsTrigger value="access" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm px-4 py-2 text-xs font-bold uppercase tracking-wider h-10 flex items-center gap-2">
            <Lock className="h-3.5 w-3.5" /> Passcodes
          </TabsTrigger>
          <TabsTrigger value="general" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm px-4 py-2 text-xs font-bold uppercase tracking-wider h-10 flex items-center gap-2">
            <SettingsIcon className="h-3.5 w-3.5" /> General Settings
          </TabsTrigger>
          <TabsTrigger value="receipt" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm px-4 py-2 text-xs font-bold uppercase tracking-wider h-10 flex items-center gap-2">
            <Printer className="h-3.5 w-3.5" /> Receipts
          </TabsTrigger>
          <TabsTrigger value="logs" className="rounded-lg data-[state=active]:bg-white data-[state=active]:shadow-sm px-4 py-2 text-xs font-bold uppercase tracking-wider h-10 flex items-center gap-2">
                        ...
                        {userProfiles.map(u => (
                          <div key={u.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-100 group transition-all hover:bg-white hover:shadow-sm">
                            <div className="flex flex-col">
                              ...
                              <span className="text-[10px] text-slate-400 font-medium whitespace-nowrap overflow-hidden text-ellipsis max-w-[120px]">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-base flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button variant="outline" size="sm" className="h-8 flex items-center gap-1.5" onClick={lockProfile}><Lock className="h-3.5 w-3.5" /> Lock Profiles</Button>
                    <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => forceSync()} disabled={!isOnline}><Globe className={`h-4 w-4 ${!isOnline ? 'text-muted-foreground' : 'text-primary'}`} /></Button>
                  </div>
                </CardTitle>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="text-sm font-medium">Admin: {currentUser.name}</div>
                  <div className="text-xs text-muted-foreground">Email: {currentUser.email}</div>
                </div>
                <div className="pt-4 border-t">
                  <Button variant="outline" className="w-full flex items-center gap-2 text-destructive hover:text-destructive hover:bg-destructive/5" onClick={logout}>
        ...
        <TabsContent value="receipt" className="animate-in fade-in slide-in-from-bottom-2">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <div>
                <CardTitle className="text-lg flex items-center gap-2"><Printer className="h-5 w-5" /> Receipt Configuration</CardTitle>
                <CardDescription>Manage business branding and branch info</CardDescription>
              </div>
              <Button onClick={handleSaveReceiptSettings} className="bg-green-600 hover:bg-green-700 h-9 px-4"><Save className="h-4 w-4 mr-2" /> Save Settings</Button>
            </CardHeader>
            <CardContent>
              <Tabs defaultValue="branch_1" className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <TabsContent value="dispatch" className="mt-0 p-4 border rounded-xl bg-slate-50/50 space-y-4">
                    <h3 className="font-bold text-xs uppercase tracking-widest text-primary">Dispatch Info</h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-1.5"><Label className="text-[10px] uppercase font-bold text-slate-500">Address</Label><Input value={formReceiptSettings.dispatchAddress} onChange={e => setFormReceiptSettings(s => ({ ...s, dispatchAddress: e.target.value }))} className="bg-white h-9" /></div>
                      <div className="space-y-1.5"><Label className="text-[10px] uppercase font-bold text-slate-500">Phone</Label><Input value={formReceiptSettings.dispatchPhone} onChange={e => setFormReceiptSettings(s => ({ ...s, dispatchPhone: e.target.value }))} className="bg-white h-9" /></div>
                    </div>
                  </TabsContent>
                </div>
              </Tabs>
        ...
        <TabsContent value="logs" className="animate-in fade-in slide-in-from-bottom-2">
          <Card>
            <CardHeader><CardTitle className="text-base flex items-center gap-2"><ShieldCheck className="h-4 w-4" /> Activity Logs</CardTitle></CardHeader>
            <CardContent>
              <div className="border rounded-xl divide-y overflow-hidden max-h-[500px] overflow-y-auto bg-white">
                {auditLogs.length === 0 ? <p className="text-xs text-muted-foreground text-center py-8">No logs recorded yet</p> : 
                  auditLogs.slice().reverse().map(log => (
                    <div key={log.id} className="p-3 text-[11px] flex justify-between items-start hover:bg-slate-50">
                      <div><p className="font-bold text-slate-900 capitalize">{log.action} {log.entity}</p><p className="text-slate-500">{log.details}</p></div>
                      <p className="text-slate-400 whitespace-nowrap">{new Date(log.timestamp).toLocaleString()}</p>
                    </div>
                  ))
                }
              </div>
