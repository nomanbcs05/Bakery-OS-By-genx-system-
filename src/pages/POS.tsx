...
import { ShoppingCart, Plus, Minus, Trash2, CreditCard, Banknote, Calculator, Tag, ChefHat, Printer } from 'lucide-react';
...
export default function POS({ branch }: POSProps) {
  ...
  const handlePrintProductsSummary = () => {
    ...
    products.filter(p => p.isActive).forEach(p => {
      const remaining = stock[p.id]?.[branch] || 0;
    ...
    <div className="animate-fade-in h-[calc(100vh-120px)]">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
        ...
        <div className="lg:col-span-2 overflow-y-auto pr-2 space-y-6" style={{ scrollbarWidth: 'thin' }}>
          {/* Branch Products Section Bar with Red and Green Print Buttons */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 p-3 bg-card border rounded-xl shadow-sm">
            <div>
              <h2 className="text-lg font-bold text-foreground flex items-center gap-2">
                Branch Products
                <Badge variant="outline" className="text-xs font-normal">
                  {availableProducts.length} in stock
                </Badge>
              </h2>
              <p className="text-xs text-muted-foreground">Available inventory and summaries for {branchLabel}</p>
            </div>
